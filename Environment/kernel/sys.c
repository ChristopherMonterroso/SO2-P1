...
SYSCALL_DEFINE1(capture_memory_snapshot, struct memory_snapshot __user *, user_snapshot)
{
    struct sysinfo info;
    struct memory_snapshot mem_snapshot;

    // Obtener información de memoria del sistema
    si_meminfo(&info);

    mem_snapshot.total_memory = info.totalram * info.mem_unit;
    mem_snapshot.free_memory = info.freeram * info.mem_unit;
    mem_snapshot.used_memory = mem_snapshot.total_memory - mem_snapshot.free_memory;
    mem_snapshot.swap_total = info.totalswap * info.mem_unit;
    mem_snapshot.swap_free = info.freeswap * info.mem_unit;

    // Copiar la información de memoria al espacio de usuario
    if (copy_to_user(user_snapshot, &mem_snapshot, sizeof(mem_snapshot))) {
        return -EFAULT; // Error al copiar al espacio de usuario
    }
...
// Variable global para contar las syscalls
static int syscall_open_count = 0;
static int syscall_write_count = 0;

// Función que intercepta la syscall 'open'
asmlinkage long tracked_sys_open(const char __user *filename, int flags, umode_t mode) {
    syscall_open_count++;
    printk(KERN_INFO "Intercepted open syscall: %s\n", filename);
    return original_sys_open(filename, flags, mode);
}

// Función que intercepta la syscall 'write'
asmlinkage long tracked_sys_write(unsigned int fd, const char __user *buf, size_t count) {
    syscall_write_count++;
    printk(KERN_INFO "Intercepted write syscall: fd=%u, count=%zu\n", fd, count);
    return original_sys_write(fd, buf, count);
}

// Reemplaza las syscalls en la tabla de syscalls
void intercept_syscalls(void) {
    original_sys_open = (void *)sys_call_table[__NR_open];
    sys_call_table[__NR_open] = (void *)tracked_sys_open;

    original_sys_write = (void *)sys_call_table[__NR_write];
    sys_call_table[__NR_write] = (void *)tracked_sys_write;
}

// Función de inicialización del módulo
static int __init syscall_intercept_init(void) {
    printk(KERN_INFO "Initializing syscall interception\n");
    intercept_syscalls();
    return 0;
}
...
static void __exit syscall_intercept_exit(void) {
    printk(KERN_INFO "Removing syscall interception\n");

    // Restaurar las syscalls originales
    sys_call_table[__NR_open] = (void *)original_sys_open;
    sys_call_table[__NR_write] = (void *)original_sys_write;
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Max Duran");
MODULE_DESCRIPTION("Módulo de rastreo de syscalls");
